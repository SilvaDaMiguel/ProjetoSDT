...
public class Element {
    ...
    private void monitorLeader() {
        while (!isLeader && running) { // Adiciona verificação de liderança
            ...
            if (leaderId != null && (currentTime - lastLeaderHeartbeat > TIMEOUT_leader)) {
    ...
    private void initiateLeaderElection() {
        ...
        try (DatagramSocket electionSocket = new DatagramSocket()) {
            ...
            // Aguarde respostas de outros elementos
            Thread.sleep(5000);

            // Se ninguém respondeu ou se este elemento tem o maior ID, torna-se líder
            boolean higherIdExists = activeElements.keySet().stream()
            ...
            if (!higherIdExists) {
                ...
                System.out.println("Outro elemento possui um ID maior. Aguardando novo líder.");
            }
