            ...
            while (true) {
                ...
                for (WatchEvent<?> event : key.pollEvents()) {
                    ...
                    if (event.kind() == StandardWatchEventKinds.ENTRY_DELETE) {
                        // Enviar mensagem de deleção
                        message = "DELETE:" + fileName.toString();
                    } else if (Files.isRegularFile(filePath) && fileName.toString().endsWith(".txt")) {
                        // Capturar eventos de criação e atualização
                        String content = Files.readString(filePath);
                        if (content == null) {
                            content = ""; // Garantir que não seja nulo
                        }
                        message = (event.kind() == StandardWatchEventKinds.ENTRY_CREATE ? "CREATE:" : "UPDATE:")
                                + fileName.toString() + ":" + content;
                    } else {
                        continue; // Ignorar eventos irrelevantes
                    }
